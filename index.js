  ...
  app.post('/removeBg', async (req, res, next) => {
    ...
    try {
      ...
      try {
        ...
        logger.error(
          `Background removal library error: ${bgRemovalError.message}`
        );
      ...
      if (height > width) {
        logger.info(
          `Image is vertical (${width}x${height}), rotating to horizontal`
        );
        ...
        logger.info(
          `Image is already horizontal (${width}x${height}), no rotation needed`
        );
      }
    ...
    try {
      ...
      if (response?.data?.aadhaarPdf) {
        try {
          ...
          logger.error(
            `Aadhaar PDF processing failed: ${processingError.message}`
          );
          res.status(500).json({
            success: false,
